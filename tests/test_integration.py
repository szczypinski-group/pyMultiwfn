...
These tests use real wavefunction files and optionally the real Multiwfn
executable.
Run with: pytest tests/test_integration.py -v -m "real_files or
requires_multiwfn"
"""
...
class TestRealMultiwfnExecution:
    ...
    #     )
    #     job = MultiwfnJob(real_wfx_file, config=config)
    #     job.add_menu(Menu.topology_search_cps)
    ...
    def test_multiple_analyses(
        ...
        config = MultiwfnConfig(
            exe_path=real_executable, working_dir=temp_dir, timeout=180
        )
        job = (
            MultiwfnJob.builder(real_wfx_file)
            .with_config(config)
    ...
    def test_wavefunction_check(
        ...
        config = MultiwfnConfig(
            exe_path=real_executable, working_dir=temp_dir, timeout=60
        )
        job = MultiwfnJob(real_wfx_file, config=config)
        job.add_menu(Menu.CHECK_WAVEFUNCTION)
        ...
        assert (
            "orbital" in result.stdout.lower()
            or "electron" in result.stdout.lower()
        )
    ...
    def test_save_and_reload_output(
        ...
        # charges = ParserRegistry.parse(
        #     "charges", saved_output, method="hirshfeld"
        # )
        # if len(charges) == 0:
...
class TestEndToEndWorkflows:
    ...
    # def test_run_all_charges_workflow(
    #     self, real_wfx_file: Path, real_executable: Path, temp_dir: Path
    # ) -> None:
    ...
    #     )
    #     job = MultiwfnJob(real_wfx_file, config=config)
    #     job.add_menu(Menu.run_all_charges)
    ...
    #     # May not fully succeed if some methods fail, but should output
    #     assert len(result.stdout) > 0
