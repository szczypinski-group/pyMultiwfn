...
def _parse_charges(stdout: str) -> dict[str, Any]:
...
def _parse_bond_orders(stdout: str) -> dict[str, Any]:
    bos = BondOrderParser.parse(stdout)
    valences = BondOrderParser.parse_valence(stdout)
    result: dict[str, Any] = {
        "bond_orders": {f"{a}-{b}": v for (a, b), v in bos.items()},
    }
    if valences:
        result["valences"] = {str(k): v for k, v in valences.items()}
    return result


def _parse_multicenter_bo(stdout: str) -> dict[str, Any]:
    mc = BondOrderParser.parse_multicenter(stdout)
    return {"multicenter_bond_orders": mc}


def _parse_bo_decomposition(stdout: str) -> dict[str, Any]:
    decomp = BondOrderParser.parse_decomposition(stdout)
    return {"bond_order_decomposition": decomp}


def _parse_critical_points(stdout: str) -> dict[str, Any]:
...
def _parse_charge_transfer(stdout: str) -> dict[str, Any]:
    return {"charge_transfer": ExcitationParser.parse_charge_transfer(stdout)}
...
def _parse_weak_interactions(stdout: str) -> dict[str, Any]:
    return {"weak_interaction_summary": WeakInteractionParser.parse(stdout)}
...
def _parse_multipole_moments(stdout: str) -> dict[str, Any]:
    return {"multipole_moments": UtilityParser.parse_multipole_moments(stdout)}
...
class ResultStore:
    ...
    def store_result(
        ...
        """
        parser_fn = _PARSER_ROUTE.get(analysis)
